...
// 1. Auth Check - Listings are restricted to logged-in users
...
try {
    ...
    // HANDLE SAVE / DELETE / DEACTIVATE (POST)
    if ($method === 'POST') {
        $override = strtoupper($input['_method'] ?? '');
        $intent   = $input['intent'] ?? ''; // 💎 Capture the intent

        if ($override === 'DELETE') {
            $result = $controller->delete($input['id'] ?? 0);
        }
        // Check for specific deactivation intent
        elseif ($intent === 'deactivate') {
            $result = $controller->deactivate($input['id'] ?? 0);
        } elseif ($intent === 'reactivate') { // 💎 Add this!
