...
class ListingPicturesController
{
    ...
    /**
     * Store multiple images for a listing (Property Photos)
    ...
    public function store($id): void
    {
        ...
        if (
            ...
            echo json_encode([
                'success' => false,
                'message' => 'No property images found in request.'
            ]);
            exit;
        }
        ...
        try {
            ...
            // 4. Check current picture count (Hard limit)
            $maxLimit = getMediaLimit();
            $currentCount = ListingPic::where('listing_id', $rawListingId)->count();
            if ($currentCount >= $maxLimit) {
                http_response_code(400);
                echo json_encode(['success' => false, 'message' => 'Limit of ' . $maxLimit . ' photos reached for this property.']);
                exit;
            }
