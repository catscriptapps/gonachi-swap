...
class UsersController
{
    ...
    public function index(): void
    {
        ...
        if (isset($_GET['q']) || isset($_GET['page'])) {
            ...
            // 💎 BRANCH: Handle Social Sidebar Search
            if ($mode === 'social') {

                // 🚀 OPTIMIZATION: Use the actual 'Follow' model
                $currentUserId = AuthService::userId();
                $followingIds = [];

                if ($currentUserId) {
                    // We pluck 'following_id' where 'follower_id' is US
                ...
                foreach ($users as $user) {
                    // Skip showing the current user in their own search results
                    if ($user->id === $currentUserId) continue;

                    $isFollowing = in_array($user->id, $followingIds);
                    $html .= self::renderSearchDropdownItem($user, $isFollowing);
                }
    ...
    public static function renderSearchDropdownItem(User $user, bool $isFollowing = false): string
    {
        ...
        $avatar = $user->avatar_url ? $assetBase . 'images/uploads/avatars/' . $user->avatar_url : null;
