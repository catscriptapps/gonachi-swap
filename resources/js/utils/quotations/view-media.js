// /resources/js/utils/quotations/view-media.js
...
/**
 * Fetches and renders gallery images for a specific quotation/project.
 * * @param {number|string} quoteId - The ID of the quotation.
 * @param {boolean|null} canManage - Permission flag. If null, it retrieves the persisted state from the DOM.
 */
export async function viewMedia(quoteId, canManage = null) {
    ...
    if (!grid._deleteAttached) {
        grid.addEventListener('click', async (e) => {
            ...
            if (btn) {
                ...
                // Use the quoteId from the outer scope
                await deleteMedia({ 
                    type: 'quotation', 
                    picId: picId, 
                    parentId: quoteId 
    ...
    try {
        const response = await fetch(`${window.APP_CONFIG.baseUrl}api/quotation-pictures?id=${quoteId}`);
        const result = await response.json();

        if (result.success) {
            // Update Count Label in Modal Header
            updateMediaCountLabel('quote-pics-count', result.pictures.length);
            ...
            result.pictures.forEach(pic => {
                const imgSrc = `${window.APP_CONFIG.baseUrl}images/uploads/quotations/${pic.pic_name}`;
                
                // 🛡️ Only generate the delete button HTML if the user has management rights
                const deleteBtnHtml = canManage ? `
                    <button type="button" data-action="delete-picture" data-id="${pic.entry_id}" class="p-2 bg-red-500/80 hover:bg-red-500 rounded-full text-white transition-colors" title="Delete Photo">
        ...
        console.error('Failed to refresh quote pictures:', error);
    }
}