...
/**
 * Fetch and render media for a specific advert.
 */
export async function viewMedia(adId, canManage = null) {
    const grid = document.getElementById('ad-pics-wrapper');
    if (!grid) return;

    // 1. Persistence: If canManage is provided (first load), save it to the DOM.
    // If it's null (on refresh), pull the last known state from the DOM.
    if (canManage !== null) {
        grid.dataset.canManage = canManage;
    } else {
        canManage = grid.dataset.canManage === 'true';
    }

    // 2. Event Listener: Attached once to the grid (Delegation)
    if (!grid._deleteAttached) {
        grid.addEventListener('click', (e) => {
            const btn = e.target.closest('[data-action="delete-picture"]');
            if (btn) {
                const picId = btn.getAttribute('data-id');
                // The media-manager.js will call viewMedia(adId) after success
                deleteMedia({ 
                    type: 'advert', 
                    picId: picId, 
                    parentId: adId 
    ...
    try {
        ...
        if (result.success) {
            ...
            result.pictures.forEach(pic => {
                ...
                // 🛡️ Use the persisted canManage state
